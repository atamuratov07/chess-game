...
import { castleMapKingMoved, castleMapRookMoved } from '../models/castle-map'
 ...
 * @param b
 */
export function performMove(b: Board, move: Move) {
	...
	if (move.capture && move.captureCoord !== move.to) {
		b.set(move.captureCoord, SPACE_EMPTY)
	}

	// If castle, we also have a rook to move:
	if (move.castleRook) {
		b.set(move.castleRookTo, spaceMarkMoved(move.castleRook))
	...
	if (
		move.capture &&
		spaceGetType(move.capture) === PIECETYPE_ROOK &&
		!spaceHasMoved(move.capture)
	) {
		current.castles = castleMapRookMoved(current.castles, move.captureCoord)
	}
